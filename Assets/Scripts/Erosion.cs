...
public class Erosion : MonoBehaviour
{
    ...
    public void Erode(ref float[,] heightMap, int mapSize, int noiseLayerSeed, float scale)
    {
        ...
        for(int x = 0; x < droplets; x++){
            for (int j = 0; j < mapSize * mapSize; j++)
            {
                float2 position = new float2(vertices[j].x, vertices[j].z);//UnityEngine.Random.Range(0.0f, mapSize - 1.0f), UnityEngine.Random.Range(0.0f, mapSize - 1.0f));
